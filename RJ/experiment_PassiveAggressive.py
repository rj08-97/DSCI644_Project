...
class my_model:
    def __init__(self):
        ...
        self.clf = PassiveAggressiveClassifier(C=0.01, class_weight="balanced", random_state=1)
    ...
    def fit(self, X, y):
        ...
        selected_feature_data = X_num[
            ['New Func >= X', 'Reached Del Func >= X', 'Top Chg by Call >= X%',
             'Top > X% by Call Chg by >= 10%', 'Top Chg Len >= X%', 'AltCountLineBlank', 'AltCountLineCode', 'AltCountLineComment',
             'CountLine', 'CountLineBlank', 'CountLineCode', 'CountLineCodeDecl', 'CountLineCodeExe', 'CountLineComment',
             'CountLineInactive', 'CountLinePreprocessor', 'CountSemicolon', 'CountStmt', 'CountStmtDecl', 'CountStmtEmpty',
             'CountStmtExe', 'SumCyclomatic', 'SumCyclomaticModified', 'SumCyclomaticStrict', 'SumEssential', 'MaxCyclomatic',
        ...
        final_data_frame = pd.DataFrame(scale_data, columns=['New Func >= X', 'Reached Del Func >= X', 'Top Chg by Call >= X%',
             'Top > X% by Call Chg by >= 10%', 'Top Chg Len >= X%', 'AltCountLineBlank', 'AltCountLineCode', 'AltCountLineComment',
             'CountLine', 'CountLineBlank', 'CountLineCode', 'CountLineCodeDecl', 'CountLineCodeExe', 'CountLineComment',
             'CountLineInactive', 'CountLinePreprocessor', 'CountSemicolon', 'CountStmt', 'CountStmtDecl', 'CountStmtEmpty',
             'CountStmtExe', 'SumCyclomatic', 'SumCyclomaticModified', 'SumCyclomaticStrict', 'SumEssential', 'MaxCyclomatic',
